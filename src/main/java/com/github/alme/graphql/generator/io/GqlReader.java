...
public class GqlReader {
	...
	public void read(GqlContext context) {
		...
		try (Reader reader = readerFactory.getReader())
		{
			...
			log.info(format(LOG_TRANSLATOR, context.getObjectTypes().size(), "Object and Input Object type"));
			...
			log.info(format(LOG_TRANSLATOR, context.getOperations().size(), "Operation"));
		}
		catch (IOException e) {
			log.error(e);
		}
